...
class Generator(metaclass=_Generator):
    ...
    def distributedbyhashproperty_sql(self, expression: exp.Property) -> str:
        expressions = self.expressions(expression, flat=True)
        order = self.sql(expression, "order")
        ...
        # [DISTRIBUTED BY HASH|RANDOM (k1[,k2 ...]) [BUCKETS num] [ORDER BY (k1[,k2 ...])]]
        return f"DISTRIBUTED BY HASH ({expressions}) BUCKETS {buckets or 'AUTO'}{order}"

    def distributedbyrandomproperty_sql(self, expression: exp.Property) -> str:
        expressions = self.expressions(expression, flat=True)
        order = self.sql(expression, "order")
        buckets = self.sql(expression, "buckets")
        return f"DISTRIBUTED BY RANDOM ({expressions}) BUCKETS {buckets or 'AUTO'}{order}"
